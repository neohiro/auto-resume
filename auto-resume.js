 ...
 *  OPENCODE_RESUME_AGGRESSION             conservative|balanced|relentless
 *                                        (balanced). One-word preset that
 *                                        sets both maxChain and
 *                                        maxStallTakeovers in tandem.
 *                                          conservative: (6, 2)  old hard cap
 *                                          balanced:     (8, 4)  default
 *                                          relentless:   (12, 8) full headroom
 *                                        Numeric env vars (below) override
 *                                        this preset if both are set.
 *  OPENCODE_RESUME_MAX_CHAIN             recovery resumes per task  (8)
 *  OPENCODE_RESUME_MAX_STALL_TAKEOVERS   fast-lane retry cap (busy-but-silent
 *                                        turn, abort+resume). The previous
 *                                        hard cap was 2, which forced you to
 *                                        babysit long free-tier stalls; raise
 *                                        it to keep auto-retrying past #2.
 *                                        Bounded orthogonally to maxChain
 *                                        (non-stall retries also advance
 *                                        chain but not stallResumes), so
 *                                        both must allow a retry.           (4)
 ...
 *  OPENCODE_RESUME_FAVORITE_CHECK_AFTER_MS  min time on alternate before
 *                                            trying to rotate back  (300000)
 *  OPENCODE_RESUME_FAVORITE_MIN_TURNS       successful turns on alternate
 *                                            before rotating back     (3)
 *  OPENCODE_RESUME_FAVORITE_RETURN          rotate back when cooldown
 *                                            expires                (true)
...
const AUTO_RESUME_VERSION = "1.16.0"
const UPDATE_URL =
 ...
 *  override the preset when both are present. conservative matches the
 *  pre-1.14 hard cap; relentless buys maximum headroom for unattended
 *  free-tier runs where the user genuinely wants the agent to keep
 *  trying. balanced is the default. */
...
const QUESTION_PATTERNS = [
  ...
  /\bprioritize (these|them|the)/i,
  /\bpick (one|the) (to start|to tackle|first|to begin)/i,
  /\bwhere (should i|would you like me) (start|begin|tackle)/i,
  /\bshould (i|we) (start|tackle|begin) with/i,
  /\bgo for (the |)(proposal|task|most impactful|first|all)/i,
...
export const AutoResumePlugin = async ({ client, $ }) => {
  ...
  const state = (id) => {
    ...
    if (!s) {
      s = {
        ...
        favoriteRotatedAt: 0, turnsSinceRotation: 0, favoriteSwapInFlight: false,
  ...
  const rotateAwayFrom = async (sessionID, reason, penalize = true) => {
    ...
    s.rlStreak = 0
    s.rotations += 1
    // Track when we rotated away so the watchdog can rotate back to the
    // user's favorite model after a cool-down. The user manually chose
    // the original model; auto-rotate should be a temporary detour, not
  ...
  /**
   * Rotate the session back to the user's favorite (originally-chosen)
   * model once the cooldown has expired and the alternate has proven
   * stable for N clean turns. The user picked the favorite deliberately;
   * auto-rotate away from a free-tier outage should be a temporary
   * detour, not a permanent downgrade.
   *
   * Returns true on rotation, false otherwise (or if the favorite is
   * still cooling, or if the session is mid-turn).
   */
  const tryRestoreFavorite = async (sessionID) => {
    if (!cfg.favoriteReturn) return false
    const s = state(sessionID)
    if (!s.originalModel) return false
    // Re-entrancy guard: a prior tick may still be awaiting the SDK swap.
    // Two concurrent swap calls would race and confuse the model state.
    if (s.favoriteSwapInFlight) return false
    const current = s.currentModel ?? s.lastModel
    if (!current) return false
    // Already on the favorite (check both currentModel and lastModel — a
    // session can be mid-tick where currentModel was just nulled out for a
    // prompt, but the most recent assistant message was on the favorite).
    if ((current.providerID === s.originalModel.providerID
         && current.modelID === s.originalModel.modelID)
        || (s.lastModel
            && s.lastModel.providerID === s.originalModel.providerID
            && s.lastModel.modelID === s.originalModel.modelID)) {
      // Reset the rotation trackers so a future rotation re-arms them.
      s.favoriteRotatedAt = 0
      s.turnsSinceRotation = 0
      return false
    }
    // Cool-down: did we rotate too recently?
    if (s.favoriteRotatedAt && Date.now() - s.favoriteRotatedAt < cfg.favoriteCheckAfterMs) return false
    // Stability: have we had enough successful turns on the alternate?
    if ((s.turnsSinceRotation ?? 0) < cfg.favoriteMinTurns) return false
    // Provider health: is the favorite still on cooldown?
    const favKey = modelKey(s.originalModel)
    if (cooling(favKey)) return false
    // Skip if the session is mid-turn — we don't want to swap models
    // while the user is actively working.
    if (s.status === "busy" || s.status === "retry") return false
    // Skip if a recovery is in flight.
    if (s.pendingResume) return false

    // All gates passed — rotate back to the favorite. Don't penalize the
    // alternate we were on; we may need it again if the favorite fails.
    // Mark swap in flight BEFORE the SDK call so concurrent ticks skip.
    s.favoriteSwapInFlight = true
    const target = s.originalModel
    try {
      log("info", "rotating back to favorite model", {
        sessionID,
        from: current ? modelKey(current) : null,
        to: modelKey(target),
        turnsOnAlternate: s.turnsSinceRotation,
      })
      // SDK first, state mutation second. rotateAwayFrom relies on the
      // next user prompt to carry the new model, so it never needs the
      // SDK call; tryRestoreFavorite runs during idle, so it MUST push
      // the model change explicitly. If the SDK call fails, the in-memory
      // state stays on the alternate and the next tick can retry cleanly.
      await client.session.chat({
        path: { id: sessionID },
        body: { providerID: target.providerID, modelID: target.modelID },
      })
      // Commit the swap only on confirmed success.
      s.currentModel = { providerID: target.providerID, modelID: target.modelID }
      s.chain = 0
      s.failStreak = 0
      s.rlStreak = 0
      s.favoriteRotatedAt = 0
      s.turnsSinceRotation = 0
      notice(`${RESUME_TAG}: Favorite model ${modelKey(target)} is back online — returning to it.`)
      queueTitleRefresh(sessionID)
      return true
    } catch (err) {
      log("warn", "favorite model swap failed", {
        sessionID,
        model: modelKey(target),
        err: err?.message ?? String(err),
      })
      // No state to roll back: s.currentModel mutation is post-await, so
      // a thrown SDK call leaves in-memory state consistent with the
      // actual server model (still on the alternate). Next watchdog tick
      // will retry the restore once the favorite's cooldown clears.
      return false
    } finally {
      s.favoriteSwapInFlight = false
    }
  }
  ...
  const schedule = (sessionID, delayMs, plan) => {
    ...
    const s = state(sessionID)
    if (s.previousKind === plan.kind && s.chain === s.previousKindChain) {
  ...
  const evaluateIdle = async (sessionID) => {
    ...
    if (!errored && hasContent) {
      ...
      // Count successful turns since the last rotation away. Used by the
      // watchdog to decide when to rotate BACK to the user's favorite
      // model — only after N clean turns on the alternate.
      s.turnsSinceRotation += 1
  ...
  const takeover = (sessionID, why, noticeMsg, plan = { kind: "resume", prompt: PROMPTS.resume }) => {
    ...
    // long free-tier stalls once the model hit retry #3. We now cap stall
    // takeovers independently via `cfg.maxStallTakeovers` (env:
  ...
  const checkStalls = () => {
    ...
    for (const [sessionID, s] of sessions) {
      ...
      // Favorite-model check: if we rotated away from the user's original
      // model (free-tier outage, quota hit, auth failure, etc.) and the
      // cooldown has now expired, try to rotate BACK. This is run on
      // every watchdog tick but the function itself has multiple gates
      // (cool-down window, min-turns stability, mid-turn protection,
      // provider-level cooldown) so it only fires when genuinely safe.
      if (s.originalModel && s.currentModel
          && (s.currentModel.providerID !== s.originalModel.providerID
              || s.currentModel.modelID !== s.originalModel.modelID)) {
        detach(tryRestoreFavorite(sessionID), `favorite-restore:${sessionID}`)
      }
