...
// ---- Q3: upstream/unavailable phrasing + AbortError are captured, with cause --
{
  ...
  ok(Boolean(resume) && resume.text.includes("Auto-resume"),
...
// ---- Q2: quality passes (improve/propose) exempt from drive-nudge budget ------
{
  ...
  await sleep(350)
  ok(state.prompts.some((p) => p.text.includes("todos unfinished")), "Q2: drive fired with budget")
...
// ---- N: todo drive -> improvement passes -> wrap-up proposals ---------------
{
  ...
  await sleep(400)
  ok(state.prompts.some((p) => p.text.includes("todos unfinished")), "N: todo-drive nudge sent")
